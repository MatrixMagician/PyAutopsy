...
install the native libraries and the package, run a single end-to-end analysis on
a disk image, and find the produced `report.html` + `report.json`.
...
below are required only for the `[ewf]` extra.
...
The install puts a `pyautopsy` command on your `PATH` (the `[project.scripts]`
...
- **`pip install ".[ewf]"` fails to build `libewf-python`** — the `[ewf]` extra
  is sdist-only and needs the native `libewf-dev` headers. Install the system
  packages in step 1, or fall back to the core `pip install .` (raw/`dd` only) if
  you do not need E01 support.
